...
国際化については[国際化とリソース](Resource.md)を参照してください。
...
- param系
  ...
  - `@param_f32v2`, `@param_f32v4`
    - (かつて `@param_pos` というのもあったが現在では非推奨、param_f32v2を使ってください)
...
そのフィルタのバージョンを指定します。
スクリプトの挙動には影響はありません。
アプリなどからそのフィルタのバージョンを表示したり、
ユーザーからのトラブルなどのコミュニケーションでどのバージョンを使っているかを明示するのに使用します。
...
- init: ウィジェットの向きの初期値。ラジアンで指定。(v1.0.10)
...
ドロップダウンから項目を選ぶ。結果は先頭を0としたインデックス（i32）。
デフォルトは0。
...
- label: ドロップダウンを説明するラベル
- items: ドロップダウンの項目。文字列配列。
