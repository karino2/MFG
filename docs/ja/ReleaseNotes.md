...
関数の中で同じsamplerを複数回使った時に重複変数定義になってスクリプトエラーになる事があるバグの修正。
...
- ドロップダウンリストをサポート（[MEP 29: ドロップダウンのウィジェット](MEP/29.md)）
- プレビューをリロードした時に位置と拡大率がリセットされていたのを保持するように修正
- DIRECTIONウィジェットにinitを追加（実装漏れの修正）
- ユーザー定義関数内でrsumなどが使われていると変数衝突が起きる事があるバグの修正


## 1.0.09 2025-12-09

ユーザーが独自の対象画像を追加できるようにする。
ヘルプメニューから開ける設定フォルダの中のpreview_targetsにpng, jpg, mdp, mdzのファイルがあれば、ターゲットとして追加する。
アプリ起動時に行うので追加後はアプリを一度終了する必要がある。
